...
type FormState = {
  ...
  ga_property_id: string; important_service_page: string
}
...
const INIT: FormState = {
  ...
  notes: '', ga_property_id: '', important_service_page: '',
}
...
export default function OnboardingForm({
  ...
  const submit = async () => {
    ...
    const res = (await response.json()) as OnboardingApiResponse
    setLoading(false)

    if (response.ok && res.success) {
      if (res.scrape_status === 'success') {
        setScrapePhase('complete')
        setScrapeMessage(res.scrape_message || 'Business context ready ✓')
      } else if (res.scrape_status === 'failed') {
        setScrapePhase('failed')
        setScrapeError(res.scrape_message || 'Context scraping failed — you can retry from the client profile.')
      } else {
        setScrapePhase('idle')
        setScrapeMessage(res.scrape_message || '')
      }
      setDone(true)
    } else {
  ...
  return (
    <Layout title="Client Onboarding">
      <style dangerouslySetInnerHTML={{
        ...
        .inp { width: 100%; padding: 9px 12px; background: var(--card); border: 1px solid var(--rule); border-radius: 8px; font-size: 14px; color: var(--ink); outline: none; font-family: var(--sans); transition: border-color .15s, box-shadow .15s; box-sizing: border-box; }
        .inp:focus { border-color: #10a37f; box-shadow: 0 0 0 3px rgba(16,163,127,.1); }
        ...
        .url-prefix { background: #1e1e1e; border: 1px solid #3f3f3f; border-right: none; border-radius: 8px 0 0 8px; padding: 9px 10px; font-size: 12px; color: #6b6b7b; font-family: var(--mono); white-space: nowrap; display: flex; align-items: center; }
        .url-input-joined { border-radius: 0 8px 8px 0 !important; }
      ` }} />
        ...
        <div style={{ display: 'flex', gap: 0 }}>
          ...
          <div style={{ flex: 1, maxWidth: 800 }}>
            ...
            <div className="section-card" id="client">
              <SectionHeader number="01" title="Client Information" desc="Business details and website for AI context scraping" icon="◈" />
              ...
              <div className="grid-2" style={{ marginTop: 20 }}>
                ...
                <Field label="Target Audience" hint="Who are you writing for?">
                  <input className="inp" placeholder="e.g. Small business owners in the Gulf" value={form.target_audience} onChange={e => set('target_audience', e.target.value)} />
                </Field>
              </div>
              ...
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <Field label="Website Domain" required error={errors.domain} hint="Main domain — AI will scrape homepage, /services, and /about automatically">
                  <div style={{ display: 'flex' }}>
                    <span className="url-prefix">https://</span>
                    <input
                      className={`inp url-input-joined${errors.domain ? ' err' : ''}`}
                  ...
                  {form.domain && (
                    <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
                      {[
                        ...
                        <span key={p.label} style={{ fontSize: 10, color: '#10a37f', background: '#0d2e26', padding: '2px 8px', borderRadius: 4, fontFamily: 'var(--mono)' }}>
                          ✓ Will scrape: {form.domain.replace(/^https?:\/\//i, '').replace(/\/$/, '')}{p.path}
                        </span>
                      ))}
                    </div>
                  )}
                ...
                <Field
                  label="Key Service Page"
                  hint="Your most important service or product page — AI reads this to understand your exact offerings"
                >
                  <div style={{ display: 'flex' }}>
                    <span className="url-prefix">https://</span>
                    <input
                      className="inp url-input-joined"
                      placeholder="example.com/ai-solutions"
                      value={form.important_service_page}
                      onChange={e => set('important_service_page', e.target.value.replace(/^https?:\/\//i, ''))}
                    />
                  </div>
                  {form.important_service_page && (
                    <span style={{ fontSize: 10, color: '#10a37f', background: '#0d2e26', padding: '2px 8px', borderRadius: 4, fontFamily: 'var(--mono)', display: 'inline-block', marginTop: 6 }}>
                      ✓ Will scrape: {form.important_service_page.replace(/^https?:\/\//i, '')}
                    </span>
                  )}
                </Field>

                {/* Scraping explanation box */}
                <div style={{ background: '#1a2e26', border: '1px solid #1f4a3a', borderRadius: 10, padding: '14px 16px' }}>
                  <div style={{ fontSize: 11, color: '#10a37f', fontWeight: 600, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
                    ⚡ AI Context Engine
                  </div>
                  <p style={{ fontSize: 12, color: '#8e8ea0', lineHeight: 1.7, margin: 0 }}>
                    After onboarding, our AI will automatically visit your website pages, read your services and content, then build a business intelligence summary. This is injected into every article prompt so content is deeply tailored to your actual business — not generic templates.
                  </p>
                  <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
                    {[
                      { icon: '🔍', text: 'Reads your pages' },
                      { icon: '🧠', text: 'Extracts services & tone' },
                      { icon: '✍️', text: 'Injects into every article' },
                    ].map(s => (
                      <div key={s.text} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <span style={{ fontSize: 13 }}>{s.icon}</span>
                        <span style={{ fontSize: 11, color: '#6b6b7b' }}>{s.text}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ marginTop: 16 }}>
                <Field label="Competitor URLs" required error={errors.competitors} hint="One URL per line — used for competitive analysis">
                  <textarea className={`inp${errors.competitors ? ' err' : ''}`} style={{ minHeight: 80 }} placeholder={'competitor1.com\ncompetitor2.com'} value={form.competitors} onChange={e => set('competitors', e.target.value)} />
                </Field>
              </div>
            </div>
            ...
            <div className="section-card" id="strategy">
              <SectionHeader number="02" title="Content Strategy" desc="Keywords, tone, and article configuration" icon="◇" />
            ...
            <div className="section-card" id="publishing">
              <SectionHeader number="03" title="Publishing" desc="Where and how often to publish" icon="↗" />
              <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <Field label="GA4 Property ID" hint="9-digit ID (e.g., 456829103) found in GA4 Property Settings.">
                  <input className="inp" placeholder="123456789" value={form.ga_property_id} onChange={e => set('ga_property_id', e.target.value.replace(/\D/g, ''))} />
                </Field>
                ...
                <Field label="Publishing Frequency" required error={errors.schedule} hint="How often should new articles be published?">
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 6 }}>
                    {SCHEDULES.map(s => (
                      <button key={s} type="button" className={`chip${form.schedule === s ? ' on' : ''}`} style={{ padding: '10px 8px', textAlign: 'center', borderRadius: 8 }} onClick={() => set('schedule', s)}>{s}</button>
                    ))}
                  </div>
            ...
            <div className="section-card" id="notes">
              <SectionHeader number="04" title="Additional Notes" desc="Optional context for better results" icon="✎" />
            ...
            {apiError && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#2a1515', border: '1px solid #5a2020', borderRadius: 10, fontSize: 13, color: '#f87171', marginBottom: 20 }}>
                <span style={{ fontSize: 16 }}>⚠</span><span>{apiError}</span>
              </div>
            )}

            <button className="btn-submit" onClick={submit} disabled={loading}>
              {loading
                ? <span style={{ display: 'flex', gap: 4 }}>{[0, 1, 2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: '#fff', animation: `dotBounce 1s ${i * 0.15}s infinite` }} />)}</span>
                : <><span>⚡</span>Start Content Automation<span>→</span></>}
            </button>
          </div>
          ...
          <aside style={{ width: 280, flexShrink: 0, padding: '0 0 0 24px', position: 'sticky', top: 180, height: 'fit-content' }}>
            <div style={{ background: 'var(--card)', border: '1px solid var(--rule)', borderRadius: 14, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  { label: 'Keywords',  value: kwCount > 0 ? `${kwCount} queued` : '—' },
                  { label: 'Niche',     value: form.niche || '—' },
                  { label: 'Tone',      value: form.tone || '—' },
                  { label: 'Frequency', value: form.schedule || '—' },
                  { label: 'Context',   value: form.domain ? '✓ Will scrape' : '—' },
                ].map(row => (
                  <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                    <span style={{ color: 'var(--muted)', fontFamily: 'var(--mono)' }}>{row.label}</span>
                    <span style={{ color: row.label === 'Context' && form.domain ? '#10a37f' : '#ececec', fontWeight: 500 }}>{row.value}</span>
                  </div>
                ))}
              </div>
            </div>

            <div style={{ background: '#0d2e26', border: '1px solid #1f4a3a', borderRadius: 12, padding: '16px 18px', marginTop: 16 }}>
              ...
              {[
                'AI scrapes your website',
                'Business context stored',
                'Keywords researched via SerpApi',
                'Article written using your context',
                'AI quality review',
                'Image generated by DALL-E',
                'Published to your site',
              ].map((step, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
                  <div style={{ width: 18, height: 18, borderRadius: '50%', background: i < 2 ? '#10a37f' : '#1f4a3a', color: '#fff', fontSize: 10, fontWeight: 700, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 1, border: i < 2 ? 'none' : '1px solid #10a37f' }}>
                    {i + 1}
                  </div>
                  <span style={{ fontSize: 12, color: i < 2 ? '#10a37f' : 'var(--ink-3)', lineHeight: 1.5, fontWeight: i < 2 ? 500 : 400 }}>{step}</span>
                </div>
              ))}
            </div>
          </aside>
...
// ── Success Screen with live scraping status ───────────────────
function SuccessScreen({
  ...
  const SERVICE_EMAIL = 'kaltech@seo-swarm.iam.gserviceaccount.com'
  ...
  const phaseSteps: { phase: ScrapePhase; label: string }[] = [
    { phase: 'fetching_pages', label: 'Visiting website pages'   },
    { phase: 'analyzing',      label: 'AI reading your content'  },
    { phase: 'saving',         label: 'Saving business context'  },
    { phase: 'complete',       label: 'Context ready'            },
  ]

  const phaseOrder: ScrapePhase[] = ['fetching_pages', 'analyzing', 'saving', 'complete']
  const currentIndex = phaseOrder.indexOf(scrapePhase)

  return (
    <Layout title="Next Steps">
      <div style={{ padding: '40px 24px', maxWidth: 680, margin: '0 auto' }}>
        <div style={{ background: '#2f2f2f', border: '1px solid #3f3f3f', borderRadius: 20, padding: 40 }}>

          {/* Header */}
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#0d2e26', border: '2px solid #10a37f', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, margin: '0 auto 16px' }}>✓</div>
            <h1 style={{ fontSize: 28, fontWeight: 300, color: '#ececec', marginBottom: 8, fontFamily: 'Fraunces, serif' }}>Onboarding Complete!</h1>
            <p style={{ color: '#8e8ea0', fontSize: 14 }}>Client <strong style={{ color: '#ececec' }}>{name}</strong> has been created and your AI pipeline is starting.</p>
          </div>

          {/* ── Live Scraping Progress ── */}
          <div style={{ background: '#1e1e1e', border: '1px solid #3a3a3a', borderRadius: 12, padding: 20, marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              {scrapePhase !== 'complete' && scrapePhase !== 'failed' && scrapePhase !== 'idle' && (
                <div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(16,163,127,0.3)', borderTopColor: '#10a37f', animation: 'spin 0.8s linear infinite', flexShrink: 0 }} />
              )}
              {scrapePhase === 'complete' && <span style={{ color: '#10a37f', fontSize: 16 }}>✓</span>}
              {scrapePhase === 'failed'   && <span style={{ color: '#f87171', fontSize: 16 }}>✕</span>}
              {scrapePhase === 'idle'     && <span style={{ color: '#6b6b7b', fontSize: 16 }}>◷</span>}
              <span style={{ fontSize: 13, fontWeight: 600, color: scrapePhase === 'complete' ? '#10a37f' : scrapePhase === 'failed' ? '#f87171' : '#ececec' }}>
                AI Context Engine
              </span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {phaseSteps.map((s, i) => {
                const stepIndex = phaseOrder.indexOf(s.phase)
                const isDone    = currentIndex > stepIndex || scrapePhase === 'complete'
                const isActive  = currentIndex === stepIndex && scrapePhase !== 'complete' && scrapePhase !== 'failed'
                const isPending = currentIndex < stepIndex && scrapePhase !== 'complete'

                return (
                  <div key={s.phase} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      width: 20, height: 20, borderRadius: '50%', flexShrink: 0,
                      background: isDone ? '#10a37f' : isActive ? '#0d2e26' : '#1e1e1e',
                      border: `1.5px solid ${isDone ? '#10a37f' : isActive ? '#10a37f' : '#3a3a3a'}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      {isDone
                        ? <span style={{ fontSize: 10, color: '#fff', fontWeight: 700 }}>✓</span>
                        : isActive
                          ? <div style={{ width: 8, height: 8, borderRadius: '50%', border: '1.5px solid transparent', borderTopColor: '#10a37f', animation: 'spin 0.8s linear infinite' }} />
                          : null}
                    </div>
                    <span style={{ fontSize: 12, color: isDone ? '#10a37f' : isActive ? '#ececec' : '#4a4a4a', fontFamily: 'DM Mono, monospace' }}>
                      {s.label}
                    </span>
                  </div>
                )
            ...
            {scrapeError && (
              <div style={{ marginTop: 12, padding: '8px 12px', background: '#2a1515', border: '1px solid #5a2020', borderRadius: 8, fontSize: 12, color: '#f87171', fontFamily: 'monospace' }}>
                ⚠ {scrapeError}
              </div>
            )}

            {scrapePhase === 'complete' && (
              <div style={{ marginTop: 12, padding: '8px 12px', background: '#0d2e26', border: '1px solid #10a37f', borderRadius: 8, fontSize: 12, color: '#10a37f' }}>
                ✓ Business context saved — every article will now be tailored to your services and brand.
              </div>
            )}
          </div>

          {/* ── GA4 Integration ── */}
          <div style={{ background: '#171717', borderRadius: 12, padding: 24, textAlign: 'left', border: '1px solid #10a37f', marginBottom: 24 }}>
            <p style={{ color: '#10a37f', fontWeight: 600, fontSize: 14, marginBottom: 12 }}>⚡ Action Required: Grant Analytics Access</p>
            <p style={{ fontSize: 13, color: '#d1d1d1', marginBottom: 16, lineHeight: 1.6 }}>
              Add this email as a <strong>'Viewer'</strong> in your Google Analytics Property Access Management to enable live analytics on your dashboard:
            </p>
            <div style={{ display: 'flex', gap: 8 }}>
              <input readOnly value={SERVICE_EMAIL} style={{ flex: 1, background: '#262626', border: '1px solid #3a3a3a', borderRadius: 6, padding: '8px 12px', color: '#10a37f', fontFamily: 'monospace', fontSize: 12 }} />
              <button onClick={copyEmail} style={{ padding: '8px 16px', background: copied ? '#0d2e26' : '#10a37f', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>
                {copied ? '✓ Copied' : 'Copy'}
              </button>
            </div>
            <p style={{ fontSize: 11, color: '#6b6b7b', marginTop: 10 }}>We only need "Viewer" access to pull organic search metrics.</p>
          </div>

          <button
            style={{ width: '100%', padding: '13px', background: '#10a37f', border: 'none', borderRadius: 10, color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}
